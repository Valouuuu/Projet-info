...
def screen_size(size): # Fonction qui permet de récupérer les dimensions de l'écran (sert à placer les objet en fonctionde la taille de l'écran et non à des coordonnées précises)
    # On prend en paramètre la dimension qui nous interesse (longeur ou largeur)
    screen_size  = pygame.display.get_desktop_sizes() 
    tu = screen_size[0]# On récupère la taille de l'écan (longeur,largeur)
    x = tu[0]
    y = tu[1]
    if size == 'x':
        return x # Retourne la longueur de l'écran
    elif size == 'y':
        return y # Retourne la largeur de l'écran
